...
class VLVRequestControl extends Control {
  ...
  ASN1Sequence toASN1() {
    ...
    if (hasTargetOffset) {
      ...
      ldapLogger.finest("VLV request Assertion value = $assertionValue");
...
class VLVResponseControl extends Control {
  ...
  VLVResponseControl.fromASN1(ASN1OctetString s) {
    ...
    ldapLogger.finest("Create control from $s  seq=${seq}");

    var x = (seq.elements.first as ASN1Sequence);

    // todo: confirm order of response
    x.elements.forEach((e) => ldapLogger.finest(" ${e.runtimeType}  $e"));
  ...
  String toString() => "${super
    .toString()}(targetpos=$targetPosition, contentCount=$contentCount,x=$extraParam)";
}
